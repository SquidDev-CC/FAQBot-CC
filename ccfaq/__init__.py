...
import ccfaq.faq_list as faq_list
...
def msg_should_process(message: discord.Message) -> bool:
...
async def faq(ctx, *, search):
    ...
    for f in faqs:
        if re.search(search, f[0]):
            results.append(discord.Embed(
                title=f[1], colour=discord.Colour(0x00e6e6), description=f[2]))
    if len(results) > 0:
        await ctx.send(content=f"I found the following {len(results)} faq(s)")
...
def score_methods(methods: Iterable[str], search: str, threshold: float = 0.8) -> Iterable[Tuple[str, float]]:
    """
...
def method_embed(method: dict, link: Callable[[dict], str]) -> discord.Embed:
...
async def search_docs(ctx, search: str, link: Callable[[dict], str]) -> None:
    ...
    if search_k in methods:
        await ctx.send(embed=method_embed(methods[search_k], link))
        return
    ...
    # next best one.
    best_matches = sorted(score_methods(methods, search_k),
    ...
    if (
        ...
        await ctx.send(
            ...
            embed=method_embed(method, link)
        )
        return
...
async def about(ctx):
    ...
    embed.add_field(
        name=":asterisk: **FAQs**",
        value=f"Currently there are {len(faqs)} FAQs loaded into memory.",
        inline=True
    )
    embed.add_field(
    ...
    embed.add_field(
        ...
        value="Bot started: `{}`\nBot uptime: `{}`".format(
            starttime.strftime("%Y-%m-%d %H:%M:%S UTC"),
